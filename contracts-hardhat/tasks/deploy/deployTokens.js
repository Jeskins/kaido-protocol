...
task("deploy-tokens", "Deploys the Tokens")
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    networks[network.name].tokens.usdc = usdcContract.address;
    ...
    networks[network.name].tokens.weth = wethContract.address;
    if (network.name === "localFunctionsTestnet") {
      return;
    }
    if (
    ...
    networks[network.name].tokens.link = linkContract.address;
    if (network.name === "localFunctionsTestnet") {
      return;
    }
    if (
    ...
    networks[network.name].tokens.usdt = usdtContract.address;
    if (network.name === "localFunctionsTestnet") {
      return;
    }
    if (
