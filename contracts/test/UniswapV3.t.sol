...
contract UniswapV3Test is Test {
    ...
    function testSinglePositionSwap() public {
        // Add liquidity //
        Slot0 memory slot0 = uniswapV3.getSlot0();
        ...
        // Burn + collect //
        {
            ...
            vm.prank(users[0]);
            (uint128 a0Collected, uint128 a1Collected) = uniswapV3.collect(
                users[1],
                tickLower,
                tickUpper,
                type(uint128).max,
    ...
    function testMultiPositionsSwap() public {
...

        {
            ...
            for (uint256 i = 0; i < addParams.length; i++) {
                ...
                (uint256 amount0, uint256 amount1) = uniswapV3.mint(
                    users[0],
        ...
        // Swap (1 for 0, exact input) //
        {
            ...
            vm.prank(users[1]);
            (int256 amount0Delta, int256 amount1Delta) = uniswapV3.swap(
        ...
        // Burn + collect //
        {
            ...
            for (uint256 i = 0; i < addParams.length; i++) {
                Position.Info memory pos = uniswapV3.getPosition(
                    addParams[i].user,
                    addParams[i].tickLower,
                    addParams[i].tickUpper
                );
                ...
                vm.prank(addParams[i].user);
                (uint256 a0Burned, uint256 a1Burned) = uniswapV3.burn(
                    addParams[i].tickLower,
                    addParams[i].tickUpper,
                    pos.liquidity
                );
